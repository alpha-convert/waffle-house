...
int64_t next_seed_sr(){
  int64_t next = *global_state.seed + global_state.odd_gamma;
  *global_state.seed = next;
  return next;
}

int64_t next_int64_sr(){
  return mix64(next_seed_sr());
}

bool next_bool_sr(){
  int64_t draw = next_int64_sr();
  return ((draw | 1L) == draw);
}

int64_t between_sr(int64_t lo,int64_t hi){
  int64_t draw = next_int64_sr();
...
int64_t non_negative_up_to_sr(int64_t max){
  int64_t draw;
  int64_t remainder;
  do {
    draw = next_int64_sr() & INT64_MAX;
    remainder = draw % (max + 1);
  } while (!remainder_is_unbiased(draw,remainder,INT64_MAX,max));
  return remainder;
}

int64_t next_int_sr(int64_t lo, int64_t hi){
  int64_t diff = hi - lo;
  int64_t result;
  if(diff == INT64_MAX){
    result = ((next_int64_sr() & INT64_MAX) + lo);
  } else if (diff >= 0){
    result = non_negative_up_to_sr(diff) + lo;
  } else {
    result = between_sr(lo,hi);
  }

  return result;
}

double unit_float_sr(){
  return unit_float_from_int64(next_int64_sr());
}


double next_float_sr(double lo, double hi){
...
CAMLprim value bool_c_sr(value sr_state_val) {
  ...
  fill_from_value(sr_state_val);
  CAMLreturn(Val_bool(next_bool_sr()));
}
...
double float_c_sr_unchecked_unboxed(value sr_state_val, double lo, double hi){
  ...
  double result = next_float_sr(lo,hi);
  return result;
}
...
CAMLprim value int_c_sr_unchecked(value sr_state_val, value lo_val, value hi_val) {
  ...
  int64_t result = next_int_sr(lo,hi);
