...
%% article.
\begin{abstract}
  Property-based testing relies on random input generators to uncover bugs, often under tight time constraints. These generators are built 
  using generator libraries, which provide expressive combinators for constructing complex inputs. 
  Although convenient,
  today's generator libraries suffer from substantial performance costs, limited by abstraction overhead and inefficient randomness libraries.
  In this paper, we identify and address these bottlenecks. We introduce \name, a multi-stage programming technique that eliminates generator overhead
  without sacrificing the flexibility and expressiveness of existing generator library functions. We augment leading generator libraries in OCaml and 
  Scala 3 to use the \name approach, preserving the original library's semantics and enabling precise, pointwise performance comparisons. We also show that \rand
  has a measurable impact on generator performance, quantify its contribution, and show that speedups of $5.5\times$ are possible. Together, these 
  improvements lead to bug-finding up to $13\times$ faster---showing that high-level generator abstractions need not come at the cost of speed. \tr{Needs a lot of work. No spice.}
\end{abstract}
...
proportion of its running time, magnifying the cost of an inefficient 
implementation.
...
\begin{enumerate}
    ...
    Finally, we show that these performance improvements extend to improved bug-finding speed, finding some bugs as much as $13\times$ faster.
\end{enumerate}
...
We used \texttt{core\_bench} 0.16 \cn in OCaml and \texttt{jmh} 0.4.7 \cn for performance microbenchmarking. For assessing and comparing PBT techniques, we used the Etna\cn platform.
...
\begin{figure}[h]
  ...
  \caption{Time to generate values of varying sizes using each \camlname strategy. Lower is better. Both axes are logarithmic. BQ is \bq; Staged SR is \camlname using \bq's \texttt{splittable\_random} RNG; Staged CSR is \camlname using \csplitmix.}
  \label{fig:time}
\end{figure}
...
well-typed STLC term generator. The type derived strategies are excluded, since
...
expect the results in this section should extend to \scalaname.
