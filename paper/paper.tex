...
\jwc{Rewrite this para, BCP what do you think now?} 
...
We begin with a quick tour through multi-stage programming, and then incrementally build \camlname, layering on features and functionality.
Last, we present the controlled experiment we use to demonstrate how faster \rand{} leads to failing faster.
...
In multi-stage programming (also known more simply as staging), programs execute
in multiple stages, with each stage producing the code to be run
in the next. For the purposes of this paper, we will
consider two stages: namely compile time and run time. In our context, staged programs
execute once at compile time, which produces more code, which is then
...
This is abstraction overhead \cite{carrette05, moller20}: the layers of abstraction that
...
For further discussion of the potential for building staged generator DSLs in other languages, see Section~\ref{section:other-langs}.
...
Values of type \texttt{code} can be combined together using an \emph{escape},
...
\jwc{BCP: You wrote a ``?'' next to this section. What's the issue?}
