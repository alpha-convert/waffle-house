...
such as the Lehmer algorithm\cn, WyHash \cn, and the xoroshiro family.
These all run between 1.2-1.6x faster per byte than SplitMix in microbenchmarks \cn.
Moreover, plenty of other optimizations on top of algorithm choice exist,
...
Our implementation in OCaml uses the preprocessor extension---``PPX''---system, but the 
approach is portable to other languages with metaprogramming capabilities. 
In Scala, for instance, the same strategy can be realized using typeclass 
