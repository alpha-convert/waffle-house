...
\jwc{
  \begin{itemize}
    \item As we discussed in Section~\ref{section:motiv}, inefficient RNG choice is a bottleneck for bugfinding power.
    \item Faster RNGs and sources of entropy exist, though they are extensionally inequivalent. To demonstrate the bugfinding power of a faster RNG, we use the natural experiment provided by OCaml's inefficient implementation of SplitMix.
    \item Unlike the previous subsections, this is not a anguage-agnostic technique. In most other PBT frameworks, the RNG operates on by machine integers, so this inefficiency does not exist.
    \item We are simply using this as a way to demonstrate that faster RNG can translate to faster bugfinding for sampling-bounded generators.
