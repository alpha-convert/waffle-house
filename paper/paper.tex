...
The crux of our binding time analysis is that particular size random seed are
only known at run time (the later stage), but the code of the generator itself
is known at compile time.

Generators --- values of type \texttt{'a Bq.t} --- are never constructed dynamically at run time in practice,
so the 
