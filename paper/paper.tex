...
Property-based testing (PBT) uses generators of random data to produce 
inputs to a system under test, which is then checked against a set of 
properties the system is expected to satisfy.  In practice, PBT is used 
...
do not themselves need to be performance experts. However, existing generator 
libraries fail to achieve good performance \jwc{sounds too absolute --- who's to say what ``good performance'' is?} due to two major sources of 
inefficiency. The first \jwc{swap these? we talk about them in the other order} is \textit{random sampling}, or the process of 
selecting random \proposechange{values}{numbers} \jwc{i'd suggest we use ``values'' to mean BSTs and things}, which, depending on one's choice of \rand and
the number of samples needed, can constitute an unexpectedly high proportion
of a generator's runtime \jwc{Nice, like this}. The second is \textit{abstraction}: while flexible 
and expressive, generator libraries rely on dynamic control structures that
allocate at runtime. Allocations compound in the aggregate, leading to costly
garbage collection pauses. \jwc{Allocation cost is a slightly OCaml-specific problem compared to the more general issue that the abstractions (binds, etc) are \emph{opaque} to the compiler, so it can't optimize the code at all.} This raises a fundamental 
question: can generator libraries produce code that is both expressive 
\textit{and} efficient? \jwc{love this end to the paragraph.}

  % in strict functionallanguages like OCaml and Scala
We outline a \proposecut{fully general}, two-stage \jwc{two-pronged? this clashes with staging i.e. metaprogramming} approach to designing generator 
libraries so that they retain their idiomatic abstractions with 
significantly improved performance \jwc{simplify: idiomatic and performant}. First, we demonstrate the impact of
...
measure the impact of this choice on generator performance. \jwc{Permute? I think staging \emph{then} randomness.} \jwc{Also, i don't love this as the first place we explain the drop-in equivalence of the RNG. It's unmotivated why wen want this.} Second, 
we propose a method of streamlining \jwc{``streamlining feels too vague here''} generator libraries using 
\textit{staged metaprogramming}, or ``staging''. \tr{Joe---maybe add
a sentence here about parsing literature?} \jwc{Maybe something like: ``Staging has long been used across domains as a technique of eliminating the overhead of idiomatic functional DSLs (cite a bunch of shit).''} Using staging, it is possible
to eliminate the overhead of many common generator abstractions. For example,
...
time---eliminating the allocation entirely. This approach is not tied to any
...
To demonstrate the effectiveness of our technique, we apply it to a leading PBT 
library, \bq, built with performance engineering in mind. \jwc{Unclear if this is true, I think we should just say that by our testing it's the fastest in OCaml.}
Our version of this library, termed \name, is a drop-in replacement for \bq which 
is completely semantically equivalent: that is, \jwc{given identical random seeds} generators written using \name 
...
sequence of values; so, differences in testing performance are directly 
attributable to \name's performance speedup.
\tr{Not really done... }
\jwc{The preceeding paragraph is \emph{really good}}

To test \name's effectiveness, we run a bunch of experiments to show that \name runs faster, allocates less, etc., than
\bq on a variety of different workloads. \jwc{Move this to previous para. I think this para should start with ``To demonstrate the generality'', paralleling the ``to demonstrate the effectiveness''}We demonstrate the generality of the technique by implementing \name in
...
In summary, we demonstrate that, across languages, monadic PBT generator DSLs can
have a significant performance overhead, and present a cross-language technique
for eliminating the overhead while preserving the idiomatic abstraction.
