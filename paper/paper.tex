...
\jwc{
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item Consider \texttt{even_c : int code -> bool code = fun cx -> .<.~cx mod 2 == 0>.} and \texttt{succ_c : int code -> int code = fun cx -> .< .~x + 1 >.}
      \item If you do \texttt{to_dyn (even_c . succ_c)}, you get \texttt{fun x -> (x + 1) mod 2 == 0}. Composing functions from code to code, and then \emph{only at the end} stamping out
      a dynamic function value ensures that the resulting code is free of abstractions.
    \end{itemize}
    and 
  \end{itemize}
}
...
\jwc{We can simplify this further by getting rid of the size parameter. Make the story even cleaner, I think!}
...
\jwc{
  \begin{itemize}
    \item In cases where the compiler cannot statically eliminate them, binds allocate short-lived closures: we allocate a closure for the continuation, and then immediately jump into it.
...
\jwc{
  \begin{itemize}
    \item The pRNG at the heart of BQ is the hottest part of the hot path: generating a single int with \texttt{int st lo hi} samples from the underlying rng many many times.
    \item BQ's prng is the SplitMix algorithm, implemented in the splittablerandom package.
    \item The precise details of how SplitMix works are unimportant for this paper, but the core of it does an enormous amount of 64-bit int arithmetic.
    \item Because of implementation details related to the garbage collector, OCaml's \texttt{int} type is 63 bits, so an \texttt{Int64.t} is actually \emph{boxed}: a pointer to a custom block, not scanned by the garbage collector.
    \item Because of this, all \texttt{Int64.t} operations (both arithmetic and bitwise operations) must allocate! A single call to \texttt{int} in Bq calls the samples the RNG many times, and each sample (call to \texttt{next\_int64}) allocates 9 words (2 multiplications, one addition, and 6 logical operations).
  \end{itemize}
}

\jwc{We discuss ways to fix this in section ...}

\jwc{We note that this inefficiency is not due to the ``abstraction overhead'' of the DSL itself, but simply due to language implementation and algorithm choices.
We include this to demonstrate the amount of slack in the system, and so we can demonstrate the additional testing-time savings in the evaluation section.
}
...
\jwc{
  \begin{itemize}
    ...
    The function application \texttt{g size_c random_c} runs during the first stage, producing the code for the \texttt{a}. This is then spliced into the quote: the specialized code of the generator \texttt{g}
