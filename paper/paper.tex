...
\tr{The locus for improvement in this area is \textit{generator libraries}: if programmers are 
 ...
 of selecting random values. The first is random sampling, or the process of selecting random values. 
 The second is abstraction: while generator libraries are designed for flexibility and expressiveness, 
 this comes at the cost of performance, resulting in frequent boxing and unboxing and unnecessary 
 allocations (which lead to costly garbage collection pauses). In strict functional languages, 
 constructs like monadic bind introduce function call overhead, requiring each step in a generator 
 sequence to allocate closures and thread state through function applications.
 }
