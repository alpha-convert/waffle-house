...
For the latter, we count the number of times a generator invokes the SplitMix sampling routine \texttt{next\_int64}. We select four strategies from \secref{subsubsection:ocaml} with disparate speedups: insertion-based BSTs, single-pass BSTs, 
...
to \texttt{bind}. The right plot shows a more bimodal distribution, but still validates our intuition that
generators that sample more also stand to gain more from optimized \rand libraries.
These results suggest that staging and \rand optimizations address distinct and complimentary roles in generator performance: the former aids combinator-heavy generators, while the latter benefits sampling-heavy ones. To support diverse workloads, generator libraries should combine both techniques.

% We conclude that staging and \rand choice play distinct and
...
overhead. These results show that staging generalizes well across languages.
