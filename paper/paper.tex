...
\lstset{
  basicstyle=\small\fontfamily{cmtt}\selectfont, % Computer Modern Typewriter
}
...
let int_pair_staged : ((int * int) code) Gen.t =
  Gen.bind (int .<0>. .<100>.) (fun cx ->
    Gen.bind (int .<0> cx) (fun cy ->
      Gen.return .<(.~cx,.~cy)>.
    )
  )
...
\subsection{A Real Monad Instance, CPS, and ``The Trick''}
