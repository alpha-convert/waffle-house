...
an impediment towards better bugfinding ability \cn \jwc{see comment for cites}.
% https://www.pcg-random.org/posts/bob-jenkins-small-prng-passes-practrand.html
% https://burtleburtle.net/bob/rand/smallprng.html


Significantly faster RNGs like the Lehmer RNG \cn exist like, and using them can
speed up generation tremendously --- in microbenchmarks, a C implementation of
the lehmer generator runs 4x faster than a C implementation of splitmix.
Similarly, PBT libraries could even eschew the requirement that a source of
entropy pass statistical tests\cn.  This is common practice in testing already:
fuzzers often simply use a buffer full of arbitrary bytes as a source of entropy
\cn.  Bumping a pointer and reading from a buffer (which can be pipelined
trivially) will always be significantly faster than any RNG with data-dependent
arithmetic instructions.

In the context of this paper, however, demonstrating the benefits of any of the
above options is tricky.  Our bug-finding evaluation's power is predicated on
