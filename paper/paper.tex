...
another impediment to finding bugs fast. While generator libraries by
...
of entropy \cn exist. \bcp{... more here?}
...
\jwc{Ensure this gets pushed to a top level contrib}
...
\subsubsection{Teasing apart speedups} \tr{Working title...}

The variation in speedups in \secref{subsubsection:ocaml} raises a natural question: what 
...
To test this idea, we look for measurable proxies that approximate a generator's reliance on its generator and randomness libraries.
...
For the latter, we count the number of times a generator invokes the sampling function \texttt{next\_int64}. We select four strategies from \secref{subsubsection:ocaml} with disparate speedups: insertion-based BSTs, single-pass BSTs, 
boolean lists, and well-typed STLC terms.

To determine how often \texttt{bind} is invoked, we generate 10,000 values using a fixed generation size of 100, and 
record the average number of \texttt{bind} calls. Results in \figref{fig:binds} show a clear linear 
correlation between the number of \texttt{bind} calls and the speedup 
achieved by staging, supporting the idea that generators that make heavier use of 
generator library functions benefit more.

\begin{figure}[h]
  \includegraphics[scale=0.4]{bind.png}
  \caption{Relationship between the number of binds---a proxy for reliance on generator library abstractions---and speedup from staging, showing that the two relate linearly.}
  \label{fig:binds}
\end{figure}

To determine the number of random samples, we repeat the same test, but use Intel ProcessorTrace \cn{} to capture a 4ms trace processor activity.
We then count the number of invocations of \texttt{next\_int64}---the actual SplitMix sampling routine---and average this over the number of complete generations in the trace. As expected,
the BST strategies, which achieve large speedups from \csplitmix, invoke the sampling function more: 353 times for the insertion-based strategy, and 271 for the single-pass strategy. 
STLC, which sees moderate improvements from \csplitmix, invokes \texttt{next\_int64} 211 times, and the boolean list strategy, which sees no improvement from \csplitmix, invokes it only 100 times\footnote{
We did not measure generator performance for \emph{un}staged generators with \csplitmix, so there is no equivalent of Figure~\ref{fig:binds} for speedup versus sampling.
}.

These results show that staging and \rand choice play distinct and
