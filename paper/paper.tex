...
\begin{abstract}
  \hg{TODO: Abstract}
  \jwc{Make sure we are clear about our results here. I.e. VERY QUICKLY in this abstract, we need to say ``finds bugs 5x faster''}
\end{abstract}
...
Property-based testing (PBT) is a software testing technique that uses
random test inputs to validate logical specifications. Recent studies on PBT usage in industry~\cite{inpractice} have shown that
...
The faster a test fails, the better.
% The goal of PBT is to fail tests as fast as possible. \jwc{First cut at ``fail fast''}
\iflater 
\bcp{Feels like we're missing a trick in the transition here.  Can we
  say, ``Because performance is important, PBT libraries are already
  quite tuned.  [Can we substantiate that??]  But more is
  possible...'' }
\fi

One important locus for performance improvement
...
that provide combinators for building and composing generators. However, careful 
measurements show that the performance of existing generator libraries falls far short of what
is possible. 

This is due to two major sources of inefficiency.
First, the same high-level design approach that gives generator libraries their
flexibility also introduces layered abstractions and indirections that are difficult for
compilers to optimize\proposecut{, shifting work to run time}.
Second, the \rand that a generator uses to make choices can
constitute an large proportion of its running time, magnifying the impact of an inefficient one. \jwc{Object of ``one'' unclear, intended to be ``\rand{}''.}
\iflater
...
In this paper, we study and address these issues.
First, we show how to use \textit{multi-stage programming}, a classic technique for 
building fast DSLs “without regret”~\cn{}, to eliminate the overhead of 
many common generator abstractions. We refer to this approach as \name.
% Secondly, we demonstrate the
% impact of diferent choices of randomness library on generator performance by conducting a
% controlled comparison.
Second, we demonstrate that a DSL's choice of \rand{} has a
dramatic impact on its performance by performing a controlled experiment,
isolating the performance benefits of faster randomness in a clearly quantifiable manner.

% These insights lay the groundwork for generator libraries that combine high-level 
% design with low-level performance.

% \bcp{It
% feels a bit grand to call what we've done an ``approach''---it's
% really just a single performance-enhancing technique, staging, plus
% the observation that the RNG matters and you should use the fastest
% one you can.}

To demonstrate the multi-language applicability of \name{}, we implement staged generator DSLs in two strict functional languages: OCaml and Scala 3.
...
In Scala 3, we build \scalaname{}, based on ScalaCheck---the language's standard PBT library\cn{}.
In both languages, our libraries preserve the semantics of the original: given the same random seed, generators written using \camlname or \scalaname produce exactly the same sequence of values as equivalent generators using \bq or ScalaCheck, respectively.
This semantic equivalence enables pointwise---as opposed to distributional---bug-finding comparisons, ensuring that if a \name{} generator finds bugs faster, this is attributable \textit{solely} to staging and \rand optimizations, not luck with seeds.
% \jwc{BCP thinks this should be a little stronger: ``and not obscured by the probablistic nature of generation'', or something like that. ``Comparing distributions of speed is hard, comparing pointwise is easier.''. This needs to have its own sentence here.}
...
by running these case studies in the Etna
platform~\cite{etna}, which simulates real-world PBT usage \iflater\hg{Don't love this phrase, I'll
think more about how we should talk about Etna}\fi by injecting bugs into a system and
measuring how quickly various generators detect them.


In summary, we show that PBT generator DSLs incur significant performance costs across languages, and present a general technique for improving their efficiency while preserving their idiomatic abstractions. \jwc{needs a bit about RNG --- BCP thoughts?}
...
\begin{enumerate}
    ...
    \item We present \name{}, a technique for eliminates the abstraction overhead of generators. We implement \name{} in both OCaml and Scala 3, showcasing its cross-language applicability.
    \item We demonstrate that writing generators using \name{} and fast \rands{} yields significant performance improvements.
    We precisely quantify the impact of each optimization separately, and explain how they play distinct yet complementary roles in improving generator performance.
    Finally, we show that these performance improvements extend to improved bug-finding speed, finding some bugs as much as \todo{\%\%} faster.
\end{enumerate}
...
develop \name{} (using \camlname{} to illustrate), and discuss how we isolate the performance of \rand{} to benchmark their impact on bug-finding speed.
...
library and copied in dozens of other frameworks,
is via an embedded {\em monadic} language. We use the syntax and types from OCaml's \bq library, which is presented in
...
generator, written the monadic syntax.
...
The random seed \texttt{SR.t} from \bq's \rand{} called \texttt{Splittable\_random}, henceforth aliased in code as \texttt{SR}.
...
for an instance of the reader monad \cn{}: \texttt{return} ignores the size and seed and returns its argument, while \texttt{bind} runs \texttt{g} and passes the result to \texttt{k}.
...
Different generator DSLs use variatoins on this basic design, but the basics are the same across the board.
...
\begin{figure}
  ...
  \end{tabular}
\caption{Microbenchmarks of Generators in \bq{} and ScalaCheck. Average over 10000 generations with random seeds and a fixed size.}
...
Unfortunately, (almost\footnote{GHC is a notable exception here, performing sophisticated list fusion optimizations \cn{}}) no compilers perform this kind of optimization.
...
Weighted union uses the Fitness Proportionate Selection algorithm \cn{}, which (1) samples a number $r$
...
a binary search. However, since the lists are short in practice, a linear scan is almost always faster \cn{}.
...
algorithm \cn, implemented by \texttt{Splittable\_random}. Meanwhile,
...
random numbers, since they usually use versions of rejection sampling \cn{} to find a value within the range.
...
they are not chosen with performance in mind, leading to worse bugfinding power than what is possible.
...
% \bcp{But this is not the point that the reader is set up to expect: we've toldthem that we're going to make generation faster, not that we're going to give them tips so that htey can make it faster by themselves}
% \jwc{This actually isn't the case. In the intro, we're very careful to say that we show that choosing a faster randomness library \emph{leads to faster bugfinding}. And we do demonstrate this.}
% \jwc{Ok --- we could get rid of this ``source of entropy'' business and just write that you can speed up any of these by pipelining your sampling --- sampling and saving multiple values at a time with SIMD instead of just one each.}
...
simply swap out the randomness library of either \bq{} or ScalaCheck one of the afformentioned faster algorithms.
...
arithmetic, we can (a) demonstrate just how much faster bugs can be found just by
using a more performant \rand{}, and (b) explore which generators benefit the most from faster \rand{}.

\section{How can I Make My Generator Library Faster?}
\label{section:faster-generators}

With a sense of the inefficiences pervasive in generator libraries, we set out in this section to investigate solutions.
We begin with a quick tour through multi-stage programming, then incrementally build \camlname, layering on features and functionality.
Last, we present the controlled experiment we use to demonstrate how a faster \rand{} leads to failing faster.

\jwc{This paragraph is a first draft, and needs to be edited!!!}
...
in multiple stages, with each stage producing the code to be run
in the next. For the purposes of this paper, we just need two stages: compile time and run time.

\paragraph{Uses of Staging in eDSL Construction}
...
the functional abstractions used to build eDSLs tend to prevent compilers from generating efficient machine code \cn.
This is \emph{abstraction overhead} \cite{carrette05, moller20}: the layers of abstraction that
make eDSLs so pleasant to use are precisely what prevents them from being fast.
...
Scala 3 \cite{scalamacros}, Haskell \cite{templatehaskell}, \jwc{Racket??}, OCaml \cite{metaocaml,macocaml}, and Java \cite{mint}.
For this paper, we have implemented staged PBT libraries both in OCaml (using MetaOCaml \cn) and Scala 3 (using
...
For further discussion of the potential for staged generator DSLs in other languages, see Section~\ref{section:other-langs}.
...
This program is known as ``the trick'' in the partial evaluation and multi-stage programming literature \cn{}.
...
The trick is best illustrated by an example. The following program reduces at compile time to \texttt{.< fun x -> (1 + x) mod 2 == 0 >.}:
\begin{lstlisting}
...
By writing DSL combinators as compile-time functions ---
and calling \texttt{eta} at the end on the completed DSL program ---  we
...
The compile-time stage will then produce simplified code, free of any DSL combinators, which can be compiled and run
...
In the staging literature, this task is known as ``binding-time analysis'' and it is more an art than a science.
...
Gen.t} as \texttt{int code -> SR.t code -> 'a code}: a compile-time
function from dynamically-known size and seed to dynamically-determined result.

The basic staged monadic generator DSL functionality can be found in Figure~\ref{fig:gen-staged-basic}.
The constant generator \texttt{return} runs at compile time. Given \texttt{cx : 'a code}, the code for an \texttt{'a}, it
returns the generator that runs \texttt{cx}. Similarly,
...
allocate lists in the hot path of the generator. Even though these lists are usually small ---
at most a few dozen elements --- each allocation takes us closer to the next garbage collection.
...
\bcp{... but they are gonig to be sad if they don't understand the details. If we're not too desperate for space, my suggestion would be to put in some more details plus sign posting that says tehy can be skipped. If we're up against space limits, then maybe most of the section should go to the appendix. Or second thought, maybe we should lean into the cmplexity here (and everywhere else) to give the impressions that what we've done is not simply applying known ideas in sraightforward ways}
\jwc{Unfortunately, I think this technique is sufficiently well-understood to staging experts that we can't claim novelty. We'll jsut cut it for space if we need it.}
...
always in \texttt{code}\footnote{This is an instance of the \emph{codensity} monad \cite{janis08}, a fact which deserves further investigation.}.
...
We can then redefine\footnote{
The type \texttt{'a Gen.t} from before is like \texttt{'a code Gen.t} with this new definition. This type change
...
individual combinators must be carefully written to ensure that \texttt{'a code} values that contain effects are \texttt{let\_insert}'d.
To ensure that the library is written correctly, we built a PBT harness to compare staged generators to their \bq equivalents over 1,000 random seeds.
By differentially testing \cn{} a large suite of generators in this way, we gained confidence that \camlname is equivalent to \bq{}.
...
Generating recursive datatypes requires recursive generators. Different generator DSLs handle defining recursive generators differently. Some allow recursive generators
to be defined as recursive functions, while others
(including both \bq and ScalaCheck) expose a fixpoint combinator to construct recursive generators.
...
they are unable to account for constraints not encoded in the type (e.g., they can derive a
tree, but not a binary search tree).
As a result, generating values with complex preconditions is statistically unlikely.
This state of affairs is not ideal, as inexperienced PBT users are liable to derive a generator, run their test harness, find no bugs, and assume the code is correct---when in reality, most values are never tested at all because they do not satisfy the preconditions.
\jwc{This paragraph reads as mostly a critique of type-deriving, and it was made already in PBT in practice...}
...
the end user does not need to understand the staging syntax---but more importantly, \name
makes it it more likely that even naïve strategies will find a bug.
\jwc{This is good --- I think we can just say that  type deriving is easier to use but less powerful, and we make it slightly more powerful, since we make it more likely that the naive strategy finds a bug.}

The type-deriving algorithm follows a compositional pattern. Generators for complex types 
are synthesized by structurally composing generators of their subtypes: 
...
a \texttt{weighted\_union} of the generators for each case; and for recursive types, 
...
Our implementation in OCaml uses the preprocessor extension (``PPX'') system \cn, but the 
approach is portable to other languages with \proposechange{metaprogramming capabilities}{type-derived generators}\jwc{I.e. if you can do type-derived generators any way, you can do them with \name{} in the same way}. 
In Scala, for instance, the same strategy \proposechange{can}{could} be realized using typeclass 
resolution. In both settings, the result is a three-stage process: a 
metaprogram---via PPX or typeclasses---constructs a generator expression 
...
executed at runtime to produce values. The entire 
pipeline remains invisible to the user. \tr{TODO: Boolean list example in a figure...}
...
implemetnation, we can precisely quantify the bugfinding speedup that a better randomness library gives across a range of PBT scenarios.

Note that while that the insight that faster sampilng translates to faster bug finding
is a cross-language one, the performance intervention described here is OCaml-specific.
In most other PBT frameworks \cn, the randomness library used operates on machine
integers, so this \emph{particular} inefficiency does not exist.

The precise details of how the SplitMix algorithm works \cn are not important for
present purposes; the key fact is that all of its operations are are
...
collect in OCaml\footnote{The OCaml GC is a generational collector \cn, and since these allocations are small and mostly very short lived, they will all be minor allocations, never to be promoted.}
...
We used \texttt{core\_bench} 0.16 \cn in OCaml and \texttt{jmh} 0.4.7 \cn for performance microbenchmarking. \tr{Etna?}

\subsection{Benchmarking Generator Speed}
\label{subsection:benchmarking}
To answer \textbf{RQ1}, we microbenchmark generators, comparing generators in \camlname to semantically identical ones in \bq,
...
in \secref{subsection:faster-rng}. Our test cases consist of generators for boolean lists, binary search trees (BST), and simply
...
\subsubsection{\camlname}
\label{subsubsection:ocaml}
...
time to generate a value (i.e., a BST, a STLC term, or a boolean list), using a random seed, varying generation sizes (10, 100, 1,000, 10,000).\footnote{The way size is used in a generator is a domain-specific implementation detail---in a list generator, size might correspond to the desired length of the list, whereas in a tree generator it refer to number of nodes, number of leaves, depth of tree, etc. Regardless of how size is encoded, our goal is to show that performance trends scale.}
We run each treatment for 5 seconds---enough for several thousand generations---and compute the average generation time of a value produced in that interval.

\begin{figure}[h]
  % \includegraphics[scale=0.33]{allocs.png}
  \includegraphics[scale=0.43]{time.png}
  \caption{Time to generate values of varying sizes using each \camlname strategy. Lower lines translate to greater performance benefit. BQ is \bq; Staged SR is \camlname using \bq's \texttt{splittable\_random} RNG; Staged CSR is \camlname using \csplitmix.}
  \label{fig:time}
\end{figure}

Our results are summarized in \figref{fig:time}.
Our type-derived \camlname BST generator achieves speedups ranging from $1.30-1.38\times$, which increase
...
staging, and its performance changes minimally when
...
For the former, we use the number of calls to \texttt{bind}---the central monadic generator function, extensively used both directly and within other combinators.
For the latter, we count the number of times a generator invokes the SplitMix sampling routine \texttt{next\_int64}. 

We select four strategies with disparate speedups: insertion-based BSTs, single-pass BSTs, 
...
record the average number of \texttt{bind} calls. To determine the number of random samples, we repeat the same test, but use Intel ProcessorTrace \cn{} to capture a 4ms snapshot of processor activity.
We then count the number of invocations of \texttt{next\_int64} and average this over the number of complete generations in the trace.

\begin{figure}[h]
  \includegraphics[scale=0.28]{bind.png}
  \caption{Left: Speedup from staging (compared to baseline \bq) versus the number of \texttt{bind} calls.
  Right: Speedup from \csplitmix (compared to staging alone) versus the number of random samples.}
  \label{fig:binds}
\end{figure}

\figref{fig:binds} summarizes our results. The left plot shows a clear linear 
...
\subsubsection{\scalaname}
In \scalaname, we implement a subset of the \camlname
strategies: the boolean list generator, single-pass BST generator, and
well-typed STLC term generator.\footnote{By default, ScalaCheck does not support deriving gnenerators from types,
so those strategies are excluded.} We did not implement an optimized version of ScalaCheck's
\rand, but our experimental setup was otherwise the same as in \secref{subsubsection:ocaml}.
...
overhead. These results show that \name generalizes well across languages.
...
\begin{figure}[h]
  \centering
  \includegraphics[scale=0.41]{scala.png}
 \caption{Time to generate values of varying sizes using each \scalaname strategy. SC is ScalaCheck; Staged is \scalaname.}
 \label{fig:scala}
 \end{figure}
...
case studies using Etna\cn{}, a platform for assessing and comparing PBT techniques.
...
\begin{figure}[h]
  \includegraphics[scale=0.31]{swarm.png}
  \caption{Distribution of individual-task speedups across strategies and benchmarks. Gray dots represent tasks. Plots with extremely low variance have no swarm overlay.}
  \label{fig:swarm}
\end{figure}

\subsubsection{Do improved type-derived generators reduce timeouts?}
...
This suggests that staged type-derived generators can provide 
significant performance improvements for free---that is, without
 any additional effort from the user.
\section{Beyond Strict Functional Languages}
\label{section:other-langs}
\jwc{Nother name for this section needed, it's mostly about strict functional languages}
...
Next, we discuss language-by-language the degree to which the staging-related insights of this paper are portable.
  ...
  \textbf{Haskell} is home to the original PBT implementation \cn{}, and
  ...
  \textbf{F\#} has both multi-stage programming capabilities \cn{} and a well-used PBT library called FsCheck \cn{}.
  The internals of FsCheck's generator eDSL are very similar to both \bq and ScalaCheck, so we expect that building a \name{}
  in F\# is primarily an engineering effort.
...
Our work is unique in its approach to speeding up PBT, but it is certainly not
the only work focused on making PBT faster.

Perhaps the best-explored approaches to speeding up bug-finding with PBT is by
changing the order in which test inputs are sampled. Feedback-driven mechanisms
like targeted PBT~\cite{loscherTargetedPropertybasedTesting2017} and
coverage-guided PBT~\cite{lampropoulosCoverageGuidedProperty2019} have the
potential to speed up testing by changing the generation order to try to find
...
try to get to bugs faster by leveraging the ``small-scope hypothesis'' and
attempting to quickly exhaust the list of small inputs to the program under
test. All of these approaches can have significant performance benefits in
practice, but they are largely orthogonal to our contributions; we expect
\name{} could be used to speed these existing techniques to varying degrees.

Similarly orthogonal to our approach are techniques for quickly filtering out
inputs that are invalid for testing. Some of these approaches work statically,
by automatically deriving generators that produce valid inputs by
...
In all cases, these approaches could be further improved by insights from \name.
...
PBT is a performance critical task. Still, despite their similar motivations and
considerations, the solutions are entirely different and complementary.
...
metaprogramming. Quasiquation-style macros were introduced to the ML family of
languages by Nielson and Nielson's Two-Level Functional Languages~\cn{} and
Taha's MetaML~\cn{}, which in turn quickly inspired MetaOCaml (and more recently
MacoCaml) in OCaml~\cn{}, Lightweight Modular Staging in Scala \cn{}, Template
Haskell in Haskell~\cn{}, and more.  In addition to implementations, researchers
...
DSLs \cite{flatt12, samth11, skthesis}.

Back in the ML-family world, the primary use of staging is building embedded DSLs
with minimal performance overhead, an idea which has come to be known as ``abstraction without regret''
...
This work on staged parsers is the closest analogue to ours. Recent PBT research has drawn an explicit link between parsing and generation, framing generators
as ``parsers of randomness'' \cite{goldsteinParsingRandomness2022}.
\jwc{Words here about the difference!!}
...
In this paper, we identified, studied, and proposed potential solutions to two
important sources of inefficiency in PBT generator libraries: abstraction
overhead and choice of \rand{}.  In the future, we hope to continue to investigate and push the boundaries of
...
about in the staging literature. For example, MetaOCaml includes a primitive floating let-insertion \cn{} that yields optimal let placement,
which we did not use in \camlname{}.
Another trick we hope to use is GADT-based techniques for unpacking the states of recursive functions. Currently, the \camlname{}
...
On the \rands{} side we plan to investigate fast \rands{} that are not equivalent to SplitMix, such as Lehmer~\cn, WyHash~\cn,
and variants of xoroshiro~\cn. We also want to try techniques to speed up sampling such as pipelining, using SIMD instructions,
or even generating random numbers ahead of time. Last, we hope to investigate the degree to which actual statistical randomness
matters in PBT.
