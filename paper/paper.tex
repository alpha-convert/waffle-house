...
\jwc{
  \begin{itemize}
    ...
    \item We define \texttt{type 'a codecps = 'z. ('a -> 'z code) -> 'z code}. This is the polymorphic CPS monad, with the continuation type always in `code'.
    ...
    \item We then define \texttt{type 'a gen = int code -> SR.t code -> 'a codecps}, which lets us have it both ways. the effectful programs like \texttt{int} do their \emph{own} let-insertion
  \end{itemize}
}
...
module CodeCps = struct
  type 'a t = 'z. ('a -> 'z code) -> 'z code

  let run (t : 'a code t) : 'a code = t (fun x -> x)

  let let_insert (cx : 'a code) : 'a code t =
    fun k -> k .< let x = .~cx in .~(k .<x>.) >.
end
