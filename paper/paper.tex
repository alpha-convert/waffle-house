...
\begin{figure}[h]
  \includegraphics[scale=0.44]{bind.pdf}
...
\begin{figure}[h]
  \centering
  \includegraphics[scale=0.68]{scala.pdf}
...
\begin{figure}[h]
  \includegraphics[scale=0.59]{speedup.pdf}
...
However, this equivalence is primarily theoretical. PBT generators are (a) not actually implemented as parsers
of random sequences, and (b) support choice-based combinators like \texttt{weighted\_union} that parsers do not.
For this reason, we must build staged generator libraries from scratch and cannot simply use existing staged parser libraries.
...
which we have modified slightly to pass deterministic seeds to all versions of a single strategy.
