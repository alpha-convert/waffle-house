...
as a lightweight, ad hoc approach to software correctness, aimed at 
detecting bugs within a short testing window---typically 50 milliseconds 
to 30 seconds. Given this practical constraint, it is imperative that 
inputs be generated as fast as possible; in other words, generators must be 
treated as performance-sensitive code.

The locus for improvement in this area is \textit{generator libraries}: if 
...
libraries fail to achieve good performance due to two major sources of 
inefficiency. The first is \textit{random sampling}, or the process of 
selecting random values, which, depending on one's choice of \rand and
the number of samples needed, can constitute an unexpectedly high proportion
of a generator's runtime. The second is \textit{abstraction}: while flexible 
...
garbage collection pauses. This raises a fundamental 
question: can generator libraries produce code that is both expressive 
\textit{and} efficient? 

  % in strict functionallanguages like OCaml and Scala
We outline a fully general, two-stage approach to designing generator 
libraries so that they retain their idiomatic abstractions with 
significantly improved performance. First, we demonstrate the impact of
...
measure the impact of this choice on generator performance. Second, 
we propose a method of streamlining generator libraries using 
\textit{staged metaprogramming}, or ``staging''. \tr{Joe---maybe add
a sentence here about parsing literature?} Using staging, it is possible
...
library,\\ \bq, built with performance engineering in mind.
Our version of this library, termed \name, is a drop-in replacement for \bq which 
is completely semantically equivalent: that is,  generators written using \name 
