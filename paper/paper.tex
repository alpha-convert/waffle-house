...
The core of any PBT generator library is a random number generator: to generate random values, we need
Different PBT libraries use a wide range of different RNGs.
Following the original Haskell QuickCheck, \bq uses the SplitMix algorithm \cn,
implemented (statefully) as an OCaml library called \texttt{Splittable\_random}. Meanwhile,
ScalaCheck uses the JSF algorithm \cn.
% https://www.pcg-random.org/posts/bob-jenkins-small-prng-passes-practrand.html
% https://burtleburtle.net/bob/rand/smallprng.html

The RNG sits at the heart of the generation hot path.
Even basic generators like generating a single \texttt{int} or \texttt{float} may sample from the RNG multiple times,
and generator combinators like \texttt{list} generate many \texttt{int}s.
Because of this, the speed of a single RNG call matters a great deal. Unfortunately,
we argue that existing PBT libraries make relatively inefficient choices on this front,
...
such as the Lehmer RNG \cn exist. In microbenchmarks, the Lehmer generator almost 2x as fast as SplitMix \cn.
...
entropy pass statistical tests like BigCrush\cn.  This is common practice in other areas of testing already:
...
In the context of this paper, however, demonstrating the benefits of any of the
...
extensionally identical: for a given seed and size parameter, they all produce
...
\subsection{Faster SplitMix with Unboxed Int}
...
This means that all operations that return an \texttt{int64} must allocate this block of memory.
...
words \jwc{this is a call to \texttt{next\_int64}}, and each allocation brings us closer
to the next garbage collection pause. While allocation in OCaml is fast and the garbage colletor
runs incrementally in small slices
\footnote{The OCaml GC is a generational collector \cn, and since these allocations are mostly very short lived, they will never be promoted. }, we will see in Section~\ref{section:eval} that this can have
a large performance impact on some generators that spend most of their time sampling data.


% \jwc{
%   \begin{itemize}
%     \item as we discussed in section~\ref{section:motiv}, 
%     \item faster rngs and sources of entropy exist, though they are extensionally inequivalent. 
%     \item here's how splitmix is slow: the core of splitmix is nextint64, which does allocations for each operation.
%     \item if we could use actual int64s, we wouldn't have to allocate.
%     \item so we do this:
%     \begin{itemize}
%       \item we implement splitmix in c (which uses normal int64s) and real int64 arithmetic, and use the ocaml ffi to call out to the c functions. t
%       \item this could be done with jane street bleeding-edge compiler extensions, though these are not yet implemented in a a version of the ocaml compiler that also has staging.
%     \end{itemize}
%   \end{itemize}
% }
