...
than their unstaged equivalents.  Further, we show generators that run
faster also find bugs faster by running these case studies in the Etna
platform~\cite{etna}, which injects bugs into a system and measures
how quickly various generators detect them.  \jwc{Beef up this
  paragraph, explain the results of our eval.}  \todo{We precisely
  quantify the impact of each intervention and explain how they play
  distinct yet complementary roles in improving generator performance.
}
...
We begin with a quick tour through multi-stage programming (\ref{xx}),
then incrementally build \camlname, layering on features and
functionality  (\ref{xx}).
Last, we present the controlled experiment we use to demonstrate how a
faster \rand{} leads to failing faster  (\ref{xx}).
