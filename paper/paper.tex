...
\jwc{We really need to have explained the effect ordering equivalence before here.}
...
\jwc{
  \begin{itemize}
    ...
    splittable random, Scalacheck uses \href{https://burtleburtle.net/bob/rand/smallprng.html}{this one}.  These
    are reasonably performant RNGs. Because this is the
    ...
    speed up generation enormously --- in our microbenchmarks, the lehmer generator runs 4x faster than splittable random.
    \item However, showing this is tricky, because different RNGs have different sampling orders, but our
...
combinators to transform \texttt{code}s.\hg{``to run at compile time rather than
run-time''? The phrase ``transform codes'' tripped me up} Deciding which types \texttt{t} can
...
to talk more about how we did this analysis?} Generators ---
...
\hg{Don't start a sentence with a lower-case letter, even one that's part of a
symbol}\texttt{return} is the constant generator, but this time it operates on
codes\hg{Is there an alternative to this phrase? Runs at compile time? Or
operates on code? I just don't like ``codes'' used this way}. Given \texttt{cx : 'a code}, the code for a \texttt{'a}, it
...
In Section~\ref{section:motiv}, we noticed that generator combinators like \texttt{weighted\_union}
...
Because we almost always know the particular list of choices at compile time, a staged version of \texttt{weighted\_union}
can generate \emph{different code} depending on the number of generators that are being unioned together.
If we use weighted union on a list (known at compile time) of generators \texttt{g1}, \texttt{g2}, and \texttt{g3},
we can emit code that never has to create a list of three elements at run time.
...
This tree of \texttt{ifs} ``searches'' for the genrator corresponding to the sampled value \texttt{r}, and then runs it.
...
that let-binds the spliced code \texttt{.\~{}(g size random)}?
...
This is all great so far, but there's a subtle issue that prevents the version of the library design discussed so far
...
that makes it so appealing van \emph{only} be used if the types involved are actually the proper monad function types.
...
discussed in the previous section!  And because the result type of \texttt{g} is
...
In prior work, this type is often referred to as the ``code generation'' monad (and sometimes called \texttt{Gen}).
...
let-binds \texttt{let x = .\~{}cx}, and then passes \texttt{.<x>.} to the continuation.
...
let recursive (x0 : 'r code) (step : ('a,'r) handle -> 'r code -> 'a code t) : 'a code t =
  fun size_c random_c -> 
    Codecps.return @@
