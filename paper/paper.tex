...
Reordering\bcp{``reordering'' sounds like rearranging a deterministic
  order; this is changing the shape of a distribution, right?} test input sampling is perhaps the most well-studied way to accelerate bug-finding in PBT.
...
\name{} could be used to speed these existing techniques to varying
degrees. \bcp{Namely: the staging part would be equally effective, but
the RNG part would be either less or not relevant.}
...
techniques from \name. \bcp{Why not also RNG improvements?  (I think
  \name{} refers {\em only} to the staging part of what we've done,
  but I'm not certain about that, and I'm not certain we've been consistent...)}
...
For this reason, we must build staged generator libraries from scratch
and cannot simply use existing staged parser libraries.  \bcp{Maybe
...
Another trick we hope to use is GADT-based techniques for unpacking the states of recursive functions\bcp{citation?}. Currently, the \camlname{}
