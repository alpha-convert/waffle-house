...
\end{lstlisting}
reduces at compile time to the more efficient \texttt{.< fun x -> (1 + x) mod 2 == 0 >.}
%
...
analysis,'' \cite{jones1993partial}, and it is more art than
science.\bcp{Why do we need to say that last part?}
...
The crux of our binding time analysis\bcp{the wording here is
  misleading: it feels like we're implying we've {\em implemented} a
  binding-time analysis...} is that the \emph{only} parts of a
...
\texttt{random} arguments and simply runs\bcp{returns the code for?} \texttt{cx}. Similarly,
...
generator. This is just a 2-argument version of ``The Trick''
